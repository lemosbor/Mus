...
for i in архивы: # для каждого архива из массива архивов
	Archive(i).extractall("/Users/Илья/Music/Приёмка") # разархивировать в папку "База"
	os.remove(i) # удалить архив
	
# НАРЕЗЧИК
папки = [] # создаём массив директорий с музыкой
for папка in Path('/Users/Илья/Music/Приёмка').iterdir():
	демо = 0 #создаём демо-трек
	for n in Path(папка).glob('*.mp3'): # в каждом файле с расширением mp3 рассматриваемой директории
	...
	демо.export("/Users/Илья/Music/"+(папка.name)+".mp3", format="mp3", tags={'artist': папка.name}) # записываем демо-трек