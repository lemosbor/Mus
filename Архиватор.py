...
существующиедемотреки=Path('/Музыка/Приёмка').glob('*.mp3').name
print(существующиедемотреки)
for папка in Path('/Музыка/Приёмка').iterdir(): #для всех подпапок в папке «Приёмка»
	if папка not in существующиедемотреки: #если для них ещё не созданы демо-треки
		print(папка)
