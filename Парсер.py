...
for i in база: # прогоняем по всем записям
	if "ссылка" not in i: # если нет ссылки на файл-архив
		...
		for i in файлообменники:
			позиция0=(str(суп).rfind(i))
			if позиция0 > 0:
				print("Ссылка найдена:", позиция0),
				подсуп = str(суп)[позиция0:] # обрезаем код страници с позиции1
...
# 				сылка найдена: 33195
# вот она: zippyshare.com/v/jsfgKo1c/file.html
# Парсим форум по альбому: ASC - Pattern Recognition: Second Sequence [2020]
# Ищем по способу 2
# Ссылка найдена: 27858
# вот она: cloud.mail.ru/public/LvnS/f2yn55rPf
# Парсим форум по альбому: Mary Lattimore - Silver Ladders [2020]
# Ищем по способу 2
# Ссылка найдена: 14315
# вот она: yadi.sk/d/0BBnHuzU-yCsqg
# Парсим форум по альбому: Teramaze - I Wonder [2020]
# Найденная по Способу 1 ссылка: ['https://www59.zippyshare.com/v/MxqZw3ef/file.html']
# Парсинг завершён. Приступаем к открытию ссылок.
		# позиция1=суп.__str__().rfind("zippyshare.com")-7 # позиция последнего словосочетания на всей странице - 7 знаков
		# позиция11=суп.__str__().rfind("yadi.sk")
		# позиция12=суп.__str__().rfind("cloud.mail")
		# print(позиция1, позиция11, позиция12, адрес)
		# if позиция1+позиция11+позиция12 < 1: break

		# подсуп = суп[позиция1:] # обрезаем код страници с позиции1
		# позиция2=[подсуп.find("html")][0]+4  # позиция первого словосочетания в подсупе + 4 знака (окончание адреса ссылки)			
		# ссылка = подсуп[:позиция2] # обрезаем обрезанный код до позиции2 
