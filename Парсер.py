...
def парсинг(адрес): # задаем функцию Парсинг, предварительно выгрузив прокси # ВОЗМОЖНО нужен метод конструктора
	global проксиадрес # использовать глобальную переменную
	global суп  # использовать глобальную переменную
...
# ПАРСИНГ ЗАПИСЕЙ
проксиадрес = FreeProxy(rand=True).get() # выгружаем первый прокси (возможно не рабочий)
...
for i in база: # прогоняем по всем записям
	if "ссылка" not in i: # если нет ссылки на файл-архив
		...
		if len(ссылка) > 0: # если ссылка есть (найдена)
			...
			файлообменники = ["zippyshare.com", "yadi.sk", "cloud.mail"] # перечисляем файлообменники, которые нас интересуют
